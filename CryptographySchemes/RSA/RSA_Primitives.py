...
class RSA():
    ...
    def constructionOfProvablePrimes(nlen:int, e:IntegerHandler, seed:IntegerHandler, hash_function:ApprovedHashFunction = ApprovedHashFunctions.SHA_512_Hash.value, bitlens:list[int]=None) -> tuple[bool, IntegerHandler, IntegerHandler]:
        ...
        success, p, p_1, p_2, pseed = RSA.provablePrimeConstruction(L,p_N_1,p_N_2,working_seed,e,hash_function)
        ...
        while q_not_determined:
            success, q, q_1, q_2, qseed= RSA.provablePrimeConstruction(L,q_N_1,q_N_2,working_seed,e,hash_function)
            if not success:
                print("q failed")
                return False, IntegerHandler(0, little_endian, 0), IntegerHandler(0, little_endian, 0)
    ...
    def  randomPrimeGeneration_ShaweTaylor(length:int, input_seed:IntegerHandler, hash_function:ApprovedHashFunction = ApprovedHashFunctions.SHA_512_Hash.value)-> tuple[bool, int, IntegerHandler, int]:
        ...
        while (prime_gen_counter <= (4 * length) + old_counter):
            ...
            for i in range (0, iterations):
                ...
                a = a + RSA.getHashValue(prime_seed_inc,hash_function) * pow(2, (i * hash_length)) #27
                # print(f"a : {a}")
            prime_seed = IntegerHandler(prime_seed.getValue() + iterations + 1, little_endian, prime_seed.bit_length)
            a = 2 + (a % (c - 3))
            z = pow(a, 2 * t, c)
    ...
    def generateRSAKeyPair_probablePrimes(security_strength: int = 112)-> tuple[RSA_PublicKey, RSA_PrivateKey]:
        ...
        while not gcd_e_phi_1:
            ...
            success, p, q = RSA.generationOfProbablePrimes(nlen, e)                   
            if success: 
                    n = IntegerHandler((p.getValue()) * (q.getValue()),little_endian)
                    p_1 = p.getValue() - 1
                    q_1 = q.getValue() - 1
...
if __name__ == '__main__':
    ...
    # public_key_gen, private_key_gen = RSA.generateRSAKeyPair(strength.security_strength, ApprovedHashFunctions.SHA_512_Hash.value)
