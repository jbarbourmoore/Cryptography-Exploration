...
void RSADurationDatapoint::writeToCSV(){
...
float generateProvableKeys(int keylength = 2048){
    ...
    int milli_seconds = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();

    float seconds = milli_seconds / 1000.0;
    ...
    milli_seconds = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();
    float encryption_seconds = milli_seconds / 1000.0;
    ...
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
    milli_seconds = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();
    float decryption_seconds = milli_seconds / 1000.0;
    ...
    std::ofstream myfile;
    // myfile.open ("rsa_durations.csv");
    myfile.open("rsa_durations.csv", std::ios::out | std::ios::app);

    myfile << keylength;
    myfile << ",";
    myfile << seconds;
    myfile << ",";
    myfile << encryption_seconds;
    myfile << ",";
    myfile << decryption_seconds;
    myfile << ",\n";
    myfile.close();
    return seconds;
