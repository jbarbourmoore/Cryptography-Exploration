...
void BigNumHelpers::xorBigNums(BIGNUM* result_pointer, BIGNUM* first_bn, BIGNUM* second_bn){
    ...
    // Transform the result into a new BIGNUM and put it in the BIGNUM result is pointing too
    BN_bin2bn(result_char_array, max_bytes, result_pointer);
}
...
PassBigNum BigNumHelpers::xorBigNums(PassBigNum first_bn, PassBigNum second_bn){
    int first_num_bytes = BN_num_bytes(first_bn.bn_);
    int second_num_bytes = BN_num_bytes(second_bn.bn_);
    ...
    unsigned char second_char_array[max_bytes];
    if (first_num_bytes == second_num_bytes) {
        BN_bn2bin(first_bn.bn_, first_char_array);
        BN_bn2bin(second_bn.bn_, second_char_array);
    } else if (first_num_bytes > second_num_bytes) {
        BN_bn2bin(first_bn.bn_, first_char_array);
        BN_bn2binpad(second_bn.bn_, second_char_array, max_bytes);
    } else {
        BN_bn2binpad(first_bn.bn_, first_char_array, max_bytes);
        BN_bn2bin(second_bn.bn_, second_char_array);
    }

    first_bn.freePassedBN();
    second_bn.freePassedBN();
    ...
    BIGNUM *result = BN_new();
    BN_bin2bn(result_char_array, max_bytes, result);
    PassBigNum result_to_pass = PassBigNum(result);
    BN_free(result);
