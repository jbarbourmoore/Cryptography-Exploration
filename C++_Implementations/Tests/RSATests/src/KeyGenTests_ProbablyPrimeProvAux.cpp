#include <gtest/gtest.h>  
#include <stddef.h>

#include "RSAPublicKey.hpp"
#include "RSAPrivateKey.hpp"
#include "RSAKeyGeneration.hpp"

/// @brief This unit tests generating a random key with bit length 2048 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_2048_stand) {
    int keylength = 2048;
    int bitlen1 = 125;
    int bitlen2 = 140;
    int bitlen3 = 130;
    int bitlen4 = 200;
    int a = -1;
    int b = 5;
    bool use_quintuple_form = false;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);
    
    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}

/// @brief This unit tests generating a random key with bit length 3076 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_3072_stand) {
    int keylength = 3072;
    int bitlen1 = 160;
    int bitlen2 = 140;
    int bitlen3 = 130;
    int bitlen4 = 150;
    int a = 5;
    int b = 3;
    bool use_quintuple_form = false;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);
    
    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}

/// @brief This unit tests generating a random key with bit length 7680 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_7680_stand) {
    int keylength = 7680;
    int bitlen1 = 300;
    int bitlen2 = 200;
    int bitlen3 = 210;
    int bitlen4 = 200;
    int a = -1;
    int b = -1;
    bool use_quintuple_form = false;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);
    
    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}

/// @brief This unit tests generating a random key with bit length 15360 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_15360_stand) {
    int keylength = 15360;
    int bitlen1 = 400;
    int bitlen2 = 250;
    int bitlen3 = 300;
    int bitlen4 = 200;
    int a = 1;
    int b = 7;
    bool use_quintuple_form = false;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);

    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}


/// @brief This unit tests generating a random key with bit length 2048 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_2048_quint) {
    int keylength = 2048;
    int bitlen1 = 125;
    int bitlen2 = 140;
    int bitlen3 = 130;
    int bitlen4 = 200;
    int a = -1;
    int b = 5;
    bool use_quintuple_form = true;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);
    
    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}

/// @brief This unit tests generating a random key with bit length 3076 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_3072_quint) {
    int keylength = 3072;
    int bitlen1 = 160;
    int bitlen2 = 140;
    int bitlen3 = 130;
    int bitlen4 = 150;
    int a = 5;
    int b = 3;
    bool use_quintuple_form = true;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);
    
    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}

/// @brief This unit tests generating a random key with bit length 7680 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_7680_quint) {
    int keylength = 7680;
    int bitlen1 = 300;
    int bitlen2 = 200;
    int bitlen3 = 210;
    int bitlen4 = 200;
    int a = -1;
    int b = -1;
    bool use_quintuple_form = true;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);
    
    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}

/// @brief This unit tests generating a random key with bit length 15360 and using it to encrypt and decrypt a message 
TEST(KeyGenTests_ProbablyPrimeProvAux, prob_with_prov_15360_quint) {
    int keylength = 15360;
    int bitlen1 = 400;
    int bitlen2 = 250;
    int bitlen3 = 300;
    int bitlen4 = 200;
    int a = 1;
    int b = 7;
    bool use_quintuple_form = true;
    const char *input_message = "";
    RSAKeyGeneration my_key_gen = RSAKeyGeneration(keylength);

    RSAKeyGenerationResult gen_res = my_key_gen.generateRSAKeysUsingProbablePrimesWithProvableAux(a, b, bitlen1, bitlen2, bitlen3, bitlen4, use_quintuple_form);
    const char *encrypted_message = gen_res.public_key_.encryptionPrimitive(input_message);
    const char *decrypted_message = gen_res.private_key_.decryptionPrimitive(encrypted_message);

    EXPECT_NE(strcmp(encrypted_message, input_message), 0);
    EXPECT_EQ(strcmp(decrypted_message, input_message), 0);

    gen_res.public_key_.freeKey();
    gen_res.private_key_.freeKey();
}